...
pub fn load_node_identity(keys: &KeysFile, node_id: &str) -> Result<NodeIdentity, Box<dyn std::error::Error>> {
    let entry = keys.nodes.get(node_id)
        .ok_or_else(|| format!("node_id '{node_id}' not found in keys fuke"))?;
    let bytes = STANDARD.decode(&entry.private_key)?;
    let arr: [u8; 32] = bytes.try_into().map_err(|_| "private key must be 32 bytes")?;
    Ok(NodeIdentity {node_id: node_id.to_string(), signing_key: SigningKey::from_bytes(&arr) })
}

pub fn load_trusted_keys(keys: &KeysFile) -> Result<HashMap<String, VerifyingKey>, Box<dyn std::error::Error>> {
...
pub fn generate_demo_keys_file(path: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for id in node_ids {
        ...
        nodes.insert(id.to_string(), NodeEntry {
    ...
    let keys_file = KeysFile { nodes, trusted_senders };
